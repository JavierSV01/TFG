...
function ProfileImageUploader ({ onUploadSuccess }) {
  ...
  const handleProfileUpload = useCallback(async () => {
    ...
    setUploadStatus({ message: 'Subiendo foto de perfil...', type: 'info' })
    const formData = new FormData()
    formData.append('avatar', selectedFile) // La clave 'avatar' debe coincidir con tu backend
    ...
    try {
      ...
      const response = await axios.post(ENDPOINTS.USER.POSTPROFILEIMAGE, formData, config)

      // Axios considera éxito si el status es 2xx
      // response.data ya contiene el cuerpo de la respuesta parseado (si es JSON)
      setUploadStatus({ message: `Foto de perfil actualizada! ${response.data.message || ''}`, type: 'success' })
      clearSelection()
      onUploadSuccess?.()
  ...
  return (
    ...
    <VStack spacing={4} align='stretch'>
      <ImageSelector
        label='Selecciona tu nueva foto'
